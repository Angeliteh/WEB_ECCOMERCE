...
import { demoProducts, demoCollections } from "@/lib/demoProducts";
import { getDemoCart, addToDemoCart, removeFromDemoCart, updateDemoCartQuantity } from "@/lib/demoCart";
...
const isShopifyConfigured = domain && key;
...
export async function createCart(): Promise<Cart> {
  // Return demo cart if Shopify is not configured
...
export async function addToCart(
  ...
  // Return demo cart with added item if Shopify is not configured
  if (!isShopifyConfigured) {
    return addToDemoCart(lines[0].merchandiseId, lines[0].quantity);
  }
...
export async function removeFromCart(
  ...
  // Return updated demo cart if Shopify is not configured
  if (!isShopifyConfigured) {
    return removeFromDemoCart(lineIds[0]);
  }
...
export async function updateCart(
  ...
  // Return demo cart with updated quantity if Shopify is not configured
  if (!isShopifyConfigured) {
    return updateDemoCartQuantity(lines[0].id, lines[0].quantity);
  }
...
export async function getCollectionProducts({
  ...
  // Return demo products if Shopify is not configured
  if (!isShopifyConfigured) {
    return {
...
export async function getCollections(): Promise<Collection[]> {
  // Return demo collections if Shopify is not configured
  if (!isShopifyConfigured) {
    return demoCollections as Collection[];
  }
...
export async function getProduct(handle: string): Promise<Product | undefined> {
  // Return demo product if Shopify is not configured
  if (!isShopifyConfigured) {
    return demoProducts.find(p => p.handle === handle) as Product;
  }
...
export async function getProductRecommendations(
  ...
  // Return demo recommendations if Shopify is not configured
  if (!isShopifyConfigured) {
    return demoProducts.slice(0, 2) as Product[];
  }
...
export async function getVendors({
  ...
  // Return demo vendors if Shopify is not configured
  if (!isShopifyConfigured) {
    return [
      { vendor: "TechBrand", productCount: 1 },
      { vendor: "FitTech", productCount: 1 },
      { vendor: "TravelGear", productCount: 1 }
    ];
  }
...
export async function getHighestProductPrice(): Promise<{
  ...
  // Return demo highest price if Shopify is not configured
  if (!isShopifyConfigured) {
    return { amount: "299.99", currencyCode: "USD" };
  }
